...
struct SegmentTree{
	...
	int find(int v, int L, int R, int len){
		push_lazy(v, L, R);
		if (L == R) return L;
		push_lazy(v*2, L, (L+R)/2); push_lazy(v*2+1, (L+R)/2+1, R);
		if (seg[v*2] >= len) return find(v*2, L, (L+R)/2, len);
		else if (fr[v*2] + fl[v*2+1] >= len)return (L+R)/2 + 1 - fr[v*2];
		else return find(v*2+1, (L+R)/2+1, R, len);
	}
} seg;
int n, m;
	...
	for (int qq=0; qq<m; ++qq){
		...
		if (ch == 'A'){
			...
			if (seg.seg[1] < x){
				...
				int p = seg.find(1, 1, n, x);
				seg.update0(1, 1, n, p, p+x-1, 1);
			}
		}
		else{
			int ul, ur; cin >> ul >> ur;
